...
def build_normalized_survey_results(
    ...
    for i, fq in enumerate(flat_questions):
        ...
        question_type = (fq.get("question_type") or "S").upper().strip()
        is_multi_select = question_type == "M"

        raw_opts = fq.get("options") or []
        if not isinstance(raw_opts, list):
            raw_opts = []
        # Options may be plain strings or dicts {option_id, text, tags} — always use the text value
        canonical_opts: List[str] = []
        for x in raw_opts:
            if isinstance(x, dict):
                canonical_opts.append(x.get("text", "") or "")
            else:
                canonical_opts.append(str(x) if x is not None else "")
